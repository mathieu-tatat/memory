<?php
	...
	if(isset($_SESSION['mismatch']) && $_SESSION['mismatch']==1 && isset($_SESSION['card1']) && isset($_SESSION['card2']) && in_array($_SESSION['card1'], range(1,2*$_SESSION['level'])) && in_array($_SESSION['card2'], range(1,2*$_SESSION['level']))){
	...
	if(isset($_POST['card_value']) && in_array($_POST['card_value'], range(1,2*$_SESSION['level']))){
		...
		&& isset($_SESSION['login']) && $_SESSION['login'] && (!isset($_SESSION['invite']) || !$_SESSION['invite'])){
			...
			?><table><td><tr><p class="victory">VICTORY!!!<p></tr></td></table><?php
		}
		unset($_POST);
	}
	?>
...
//menu selections
	...
	<div id="menu_area">
		<?php
			if(!isset($_SESSION['connected']) && !isset($_POST['menu1']) && !isset($_POST['menu2']) && !isset($_POST['menu3']) && !isset($_POST['menu4']) && !isset($_POST['menu5'])){
		?>
...
//choix de partie
	...
	?>
		<div id="register_form" class="form_menu">
			<form method="post" action="index.php">
				...
				<input type="text" name="login" placeholder="Ex: John-Doe64" required><br>
				<label for="password">Mot de passe :<br></label>
				<input type="password" name="password" required><br>
				<label for="vpassword">Confirmer le mot de passe :<br></label>
				<input type="password" name="vpassword" required><br>
	...
	<?php
		...
		//Si la partie n'est pas creée
			...
			?>
				...
				<form method="post" action="index.php" class="form_menu"></br>
					<label for="level-select">niveau de difficulté:</label></br>
						<select class="menuRoul" name="level" id="level-select">
							<option value="">--Choisir nombre de paires--</option></br>
						...
						<label for="time-select">Temps d'affichage cartes:</label></br>
						<select class="menuRoul" name="time" id="time-select"></br>
							<option value="">--choisir une durée--</option></br>
					...
					<input type="checkbox" checked hidden name="generate"><br>
					<input class="menu_submit" type="submit" value="Jouer">
				</form>
			<?php
		}
	...
	<?php 
		...
		?>
			<p>Voulez-vous vraiment annuler la parite ?</p>
			<form method="post" action="index.php" class="form_menu">
